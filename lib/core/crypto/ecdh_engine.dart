...
class ECDH {
  ...
  static Uint8List computeSharedSecret(String privateKeyHex, String publicKeyHex) {
    try {
      if (!_isValidHex(privateKeyHex) || !_isValidHex(publicKeyHex)) throw Exception('Invalid hex format');

      final d = BigInt.parse(privateKeyHex, radix: 16);
      if (d <= BigInt.zero || d >= Secp256k1Constants.n) throw Exception('Invalid private key range');

      final pubBytes = Uint8List.fromList(HEX.decode(publicKeyHex));
      final fullPubBytes = pubBytes.length == 32 ? Uint8List.fromList([0x02, ...pubBytes]) : pubBytes;
  ...
  static bool _isValidHex(String hex) => hex.isNotEmpty && RegExp(r'^[0-9a-fA-F]+$').hasMatch(hex);
}